...
const Login = () => {
    ...
    const handleAuth = async (e: React.SubmitEvent) => {
        ...
        try {
            ...
            setError(err.message || (isRegistering ? 'Falha ao registrar.' : 'Falha ao fazer login.'));
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <main className="min-h-screen flex w-full bg-surface font-body text-on-surface selection:bg-primary-container selection:text-on-primary-container overflow-x-hidden" style={{ minWidth: '320px' }}>
                        ...
                        <div className="bg-surface-container-lowest rounded-xl login-card-shadow p-6 sm:p-10 border border-outline-variant/5">
                            ...
                            <form className="space-y-6" onSubmit={handleAuth}>
                                ...
                                {!isRegistering && (
                                    <div className="flex items-center justify-between pt-2">
                                        <label className="flex items-center gap-3 cursor-pointer">
                                            <input className="w-4 h-4 rounded-sm border-surface-variant text-primary focus:ring-primary focus:outline-none" type="checkbox" />
                                            <span className="text-sm text-on-surface-variant">Lembrar dispositivo</span>
                                        </label>
                                        <button type="button" className="text-sm text-primary font-semibold hover:underline decoration-2 underline-offset-4 bg-transparent border-none cursor-pointer">
                                            Esqueceu a senha?
                                        </button>
                                    </div>
                                )}
                                ...
                                <button
                                    ...
                                    disabled={isLoading}
                                >
                                    {isLoading ? (isRegistering ? 'Criando conta...' : 'Autenticando...') : (isRegistering ? 'Registrar' : 'Autenticar')}
                                </button>

                                <div className="text-center pt-4">
                                    <p className="text-xs text-on-surface-variant">
                                        {isRegistering ? 'Já possui acesso? ' : 'Precisa de acesso? '}
                                        <button
                                            type="button"
                                            onClick={toggleMode}
                                            className="text-primary font-bold hover:underline bg-transparent border-none cursor-pointer p-0 m-0 text-xs"
                                        >
                                            {isRegistering ? 'Fazer login' : 'Solicitar credenciais'}
                                        </button>
                                    </p>
                                </div>
                                ...
                                <div className="flex items-center my-6">
                                    ...
                                    <span className="px-4 text-xs text-on-surface-variant uppercase tracking-wider">ou continuar com</span>
                                    <div className="flex-1 bg-outline-variant/30 h-px"></div>
                                </div>
